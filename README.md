...
The wrapper exposes the onpremise module's selectable metrics collector. The
backend flags install components independently, while `metrics_collector`
selects exactly one scraper:

- `prometheus.enabled = true` keeps kube-prometheus-stack installed and owns
  the Prometheus `PodMonitor`/`ServiceMonitor` CRDs.
- `victoria_metrics.enabled = true` installs the VictoriaMetrics storage/query
  cluster without a cluster-wide controller.
- `victoria_metrics.operator.enabled = true` installs the VictoriaMetrics
  Operator chart `0.67.2`, its CRDs/RBAC, and Operator-managed resources. The
  gate defaults to `false`.
- `metrics_collector = "prometheus"` keeps the Prometheus server active and
  remote-writes a validation copy to VictoriaMetrics.
- `metrics_collector = "victoria_metrics"` requires both VictoriaMetrics gates,
  disables Prometheus server scraping, and creates one Operator-managed
  `VMAgent`.

Keep both backends and the VM Operator enabled for the converter-based rollout.
In Victoria-selected mode Prometheus remains installed for CRD ownership, but
is not an active scraper. When only the VictoriaMetrics backend is wanted for
Prometheus remote-write/query validation, leave `operator.enabled = false`.
...
Apply this Prometheus-first state first. Verify the Prometheus remote-write
queue is drained with
`max(prometheus_remote_storage_samples_pending) == 0` and that failed/retried
counters do not increase. Grafana can query both datasources; use the
VictoriaMetrics datasource to verify the copied application and
kube-state-metrics series.

Then change only `metrics_collector` to `"victoria_metrics"` and apply during
an agreed handoff window. The Operator converts application
`PodMonitor`/`ServiceMonitor` resources, including authorization Secret
selectors, and the module creates a native kube-state-metrics
`VMServiceScrape` named `<Service fullname>-victoria-metrics`, distinct from
the Operator-converted source monitor during handoff. If a caller supplies an inline job named exactly
`kube-state-metrics`, that job is preserved and suppresses the native object.
Do not place token/password values in `agent.extra_scrape_configs`; keep auth
in application-owned monitor Secret selectors.

The pinned Operator chart's chart-owned ClusterRole grants cluster-wide
wildcard verbs on `secrets` and `secrets/finalizers`. Restrict and audit the
Operator service account. Terraform does not read source Secret values; the
Operator writes generated VMAgent configuration to its own Secret at runtime.

After the switch, require
`max(vmagent_remotewrite_pending_data_bytes) == 0`, healthy targets, the
required application metrics, and no increase in VMAgent connection-read,
connection-write, dial, retry, or dropped-packet counters. Confirm exactly one
active scraper. Roll back by changing only the selector to `"prometheus"`
while both flags stay enabled. The cluster release, vmstorage PVC identities,
retention, and historical samples remain unchanged; a switch can create a
bounded overlap/gap and does not backfill missing samples.

The wrapper forwards `metrics_collector` and the complete `victoria_metrics`
object unchanged. Its checked-in local child source is only for sibling-module
validation. Registry-source restoration is blocked until an exact published
`dasmeta/grafana/onpremise` version containing this contract is supplied; do
not guess or uncomment a version before that release exists.
...
## S3 public ACL handling

For the module-managed Loki and Tempo buckets, `ignore_public_acls` is exposed directly on the existing bucket-related inputs:
...
| <a name="input_tempo"></a> [tempo](#input\_tempo) | confgis for tempo deployment | <pre>object({<br/>    enabled                 = optional(bool, false)<br/>    namespace               = optional(string, null) # the namespace fallbacks to var.namespace if not specified<br/>    create_namespace        = optional(bool, true)   # whether create namespace if not exist<br/>    chart_version           = optional(string, "1.23.3")<br/>    bucket_name             = optional(string, "")<br/>    restrict_public_buckets = optional(bool, true)<br/>    block_public_acls       = optional(bool, true)<br/>    block_public_policy     = optional(bool, true)<br/>    ignore_public_acls      = optional(bool, true)<br/>    enable_service_monitor  = optional(bool, true)<br/>    storage = optional(object({<br/>      backend               = optional(string, "s3")<br/>      backend_configuration = optional(map(any), {})<br/>    }), {})<br/>    service_account = optional(object({<br/>      name        = optional(string, "tempo")<br/>      enable      = optional(bool, true)<br/>      annotations = optional(map(string), {})<br/>    }), {})<br/>    metrics_generator = optional(object({<br/>      enabled    = optional(bool, true)<br/>      remote_url = optional(string, "http://prometheus-kube-prometheus-prometheus.monitoring.svc.cluster.local:9090/api/v1/write")<br/>    }))<br/><br/>    persistence = optional(object({<br/>      enabled       = optional(bool, true)<br/>      size          = optional(string, "20Gi")<br/>      storage_class = optional(string, "gp2") # TODO: consider switching to gp3 or ever better leave empty to use default storage class<br/>    }), {})<br/>    extra_configs = optional(any, {}) # allows to pass extra/custom configs to tempo helm chart, this configs will deep-merged with all generated internal configs and can override the default set ones. All available options can be found in for the specified chart version here: https://artifacthub.io/packages/helm/grafana/tempo?modal=values<br/>  })</pre> | `{}` | no |
| <a name="input_victoria_metrics"></a> [victoria\_metrics](#input\_victoria\_metrics) | Values for the VictoriaMetrics cluster, Operator, and selected VMAgent collector. | <pre>object({<br/>    enabled          = optional(bool, false)<br/>    namespace        = optional(string, null) # the namespace fallbacks to var.namespace if not specified<br/>    create_namespace = optional(bool, true)   # whether create namespace if not exist<br/>    chart_version    = optional(string, "0.31.0")<br/>    release_name     = optional(string, "victoria-metrics")<br/>    retention_period = optional(string, "30d")<br/>    vmstorage = optional(object({<br/>      replica_count = optional(number, 3)<br/>      storage_class = optional(string, "")<br/>      storage_size  = optional(string, "100Gi")<br/>      access_modes  = optional(list(string), ["ReadWriteOnce"])<br/>    }), {})<br/>    vminsert = optional(object({<br/>      replica_count = optional(number, 2)<br/>    }), {})<br/>    vmselect = optional(object({<br/>      replica_count = optional(number, 2)<br/>    }), {})<br/>    operator = optional(object({<br/>      enabled       = optional(bool, false)<br/>      chart_version = optional(string, "0.67.2")<br/>      release_name  = optional(string, "victoria-metrics-operator")<br/>      extra_configs = optional(any, {})<br/>    }), {})<br/>    agent = optional(object({<br/>      name                 = optional(string, "victoria-metrics-agent")<br/>      replica_count        = optional(number, 1)<br/>      extra_scrape_configs = optional(any, [])<br/>      extra_configs        = optional(any, {})<br/>    }), {})<br/>    extra_configs = optional(any, {})<br/>  })</pre> | `{}` | no |
